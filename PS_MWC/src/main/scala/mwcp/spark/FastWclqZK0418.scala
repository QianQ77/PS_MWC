...
/**
  ...
  */
object FastWclqZK0418 extends LogHelper with Watcher with Serializable{
  ...
  def fastWClq_ZK(zooKeeperServerIp: String, subproblem: Subproblem,
    ...
    if (startSetSize == 0) {
      ...
      if (startSet_weight_sum + partition_path_weight_sum <= ZKHelper.readWeightFromZookeeper(zk, this)) {
        None
        ...
        while (iterations < MAX_ITERATION && startSetSize > 0) {
          ...
          */
          ...
          while (candidatesSet.size != 0) {
            ...
            if (curr_weight + getWeightForVid(weightMap_bc, addingV) +
              getUpperBoundForVid(addingV, neighborsMap_bc, weightMap_bc,  candidatesSet) + partition_path_weight_sum > zk_mwcl) {
          ...
          // use current zk_mwcl - partition_path_weight_sum to reduce graph
          if (curr_weight + partition_path_weight_sum > zk_mwcl && curr_weight + partition_path_weight_sum > sub_mwcl.weight) {
